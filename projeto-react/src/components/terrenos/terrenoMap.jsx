...
const TerrenoMap = ({ id, selectedMarcoId, selectedConfrontacao }) => {
    ...
    const fetchTerreno = async () => {
      try {
        ...
        if (terrenoDoc.exists()) {
          ...
          // Fetch area and set it in state
          if (terrenoData.area) {
            setArea(terrenoData.area);
          } else {
            console.error("Area not found in terreno data:", terrenoData);
          }
        } else {
  ...
  useEffect(() => {
    if (scriptLoaded) {
      ...
      if(area){
        mapInstance.setZoom(getZoomLevelForArea(area));
      }
